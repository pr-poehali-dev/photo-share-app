...
export default function PhotoModal({ photo, onClose, onLike, onPrev, onNext, hasPrev, hasNext }: PhotoModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center animate-scale-in"
      onClick={onClose}
    >
      {/* Backdrop */}
        ...
        <div className="lg:w-72 bg-[hsl(220,18%,9%)] p-6 flex flex-col justify-between">
          <div>
            ...
            <div className="flex justify-between items-start mb-6">
              <span className="text-xs px-3 py-1 rounded-full bg-white/8 border border-white/10 text-white/60 font-body">
                {photo.category}
              </span>
              <button
                onClick={onClose}
            ...
            <p className="text-muted-foreground text-sm font-body mb-1">{photo.author}</p>
          ...
          <div className="mt-6 space-y-4">
            ...
            >
              <Icon
                name="Heart"
                size={18}
                className={`transition-all ${photo.liked ? 'fill-rose-400 text-rose-400' : ''}`}
              />
            ...
            <button className="w-full flex items-center justify-center gap-2 p-3 rounded-xl bg-white/4 border border-white/6 text-white/60 hover:text-white hover:border-white/15 transition-all text-sm font-body">
